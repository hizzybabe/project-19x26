...
import type { EnemyClass, ItemSlot, Rarity, RoomKind, TelegraphProfile } from './types'
...
export interface MonsterDefinition { id: string; name: string; enemyClass: EnemyClass; glyph: string; levelOffset: number; attackIntervalMs: number; dodge: number; telegraph?: TelegraphProfile }
...
export function loadContent(raw: Record<keyof ContentCatalog, unknown>): ContentCatalog {
  ...
  const telegraph = (value: unknown, path: string): TelegraphProfile => {
    const x = object(value, path)
    keys(x, path, ['name', 'windupMs', 'damageMultiplier', 'intervalMs', 'firstDelayMs'])
    return { name: text(x.name, `${path}.name`), windupMs: integer(x.windupMs, `${path}.windupMs`, 1), damageMultiplier: number(x.damageMultiplier, `${path}.damageMultiplier`, 0.001), intervalMs: integer(x.intervalMs, `${path}.intervalMs`, 1), firstDelayMs: integer(x.firstDelayMs, `${path}.firstDelayMs`, 1) }
  }
  const monsters = collection(raw.monsters, 'monsters', (value, path): MonsterDefinition => {
    const x = object(value, path)
    keys(x, path, ['id', 'name', 'enemyClass', 'glyph', 'levelOffset', 'attackIntervalMs', 'dodge'], ['telegraph'])
    const offset = number(x.levelOffset, `${path}.levelOffset`, -99)
    if (!Number.isSafeInteger(offset)) invalid(`${path}.levelOffset`, 'must be an integer')
    const dodge = number(x.dodge, `${path}.dodge`)
    if (dodge > 1) invalid(`${path}.dodge`, 'must be <= 1')
    return {
      id: id(x.id, `${path}.id`), name: text(x.name, `${path}.name`),
      enemyClass: oneOf(x.enemyClass, `${path}.enemyClass`, ['Minion','Normal','Veteran','Elite','Boss'] as const),
      glyph: text(x.glyph, `${path}.glyph`), levelOffset: offset,
      attackIntervalMs: integer(x.attackIntervalMs, `${path}.attackIntervalMs`, 1), dodge,
      ...(x.telegraph === undefined ? {} : { telegraph: telegraph(x.telegraph, `${path}.telegraph`) }),
    }
  })
